    ...
    try {
      ddb.get(companyQueryParams, async function(err, data) {
        if(err) {
          ...
          if(!result_companyId) {
            await createAllSuperAdminOp({
              ...event.request.userAttributes, companyId, profileId 
            }, dateNow);
          } else {
            console.log("Azienda già esistente, controllo il profilo");
            const profileQueryParams = {
              TableName: `${process.env.PROFILETABLE}`,
              Key: { id: profileId }, 
            };

            ddb.get(profileQueryParams, async function(err, data) {
              if(err) {
                console.log('Errore nella ricerca dell\'azienda', err.stack);
                return null;
              } else {
                const result_profile = (data && data.Item) ? data.Item.id : null;
                if(!result_profile) {
                  await createProfileSuperAdminOp({
                    ...event.request.userAttributes, profileId,
                    companyId,
                  }, dateNow);
                } else {
...
async function createProfileSuperAdminOp(attributes, dateNow) {
  const profileParams = {
    Item: {
      __typename:'Profile',
...
async function createAllSuperAdminOp(attributes, dateNow) {
  ...
  const companyParams = {
    Item: {
      ...
      location: {
        city: attributes["custom:companyCity"],
        address: attributes["custom:companyAddress"],
        province: attributes["custom:companyProvince"],
  ...
  await createProfileSuperAdminOp(attributes, dateNow);
