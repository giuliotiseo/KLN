...
import { changePreOrdersListDateRange, changePreOrdersListLimit, DEFAULT_PREORDER_LIMIT } from '../../slices/preOrdersListSlice';

function PreOrdersListOptions({
  dateFrom,
  dateTo,
  pagination,
  limit = DEFAULT_PREORDER_LIMIT,
  refetch
}) {
      ...
      <div className='flex items-center flex-1'>
        <div className="flex flex-1">
          <RangeMonthsPicker
            dateFrom={dateFrom}
            dateTo={dateTo}
            updateMonthDateRange={(value) => dispatch(changePreOrdersListDateRange(value))}
        ...
        <LimitSelector
          limit={limit}
          className="mr-2"
          updateLimit={(value) => dispatch(changePreOrdersListLimit(value))}
        />
      </div>
...
export default PreOrdersListOptions
