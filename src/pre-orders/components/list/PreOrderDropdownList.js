...
const COMPANY_TYPE_LABEL = {
  ...
  STORAGE: "Lista deposito",
}

function PreOrderDropdownList() {
  ...
  return (
    <Dropdown
      id="pre-orders-menu-list-type-dropdown"
    ...
    >
      <DropdownList id="pre-orders-menu-list-type-dropdown-list">
        ...
        <DropdownListItem className="flex items-center contact-menu-dropdown-list-item-1 hover:bg-base-200">
          <Button
            text="Lista deposito"
            className={`${companyType === "STORAGE" ? 'font-bold text-primary-200 dark:text-primary-300' : ''}`}
            onClick={() => setSearchParams({
              status: statusType,
              company: "STORAGE"
            })}
...
export default PreOrderDropdownList
