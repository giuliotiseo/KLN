...
// Main component ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
export default function EditOrderBySenderContainer({ queryFrom }) {
  ...
  */
  ...
  if(selectedOrder?.tenantSender !== myCompany.tag) {
    toast.error("Non disponi dell'autorizzazione per modificare gli ordini");
    return <Navigate to={`/orders?from=${queryFrom}&status=pending`} />
  }
        ...
        <div className="relative col-span-2 row-span-2 md:row-span-3 md:col-span-1">
          <SafeCol id="order-editor-right" ref={scrollableRef_right}>
            <div className="relative w-full h-[250px] mb-4 mt-2 pl-1 rounded-md overflow-hidden">
              <PickupDeliveryMap pickup={order.pickupCheckpoint} delivery={order.deliveryCheckpoint} />
            </div>

            <div className="bg-base-100 mt-2 mb-4 ml-1 px-4 py-4 rounded-md">
              <CarrierVisualizer name={order.carrierName} tag={order.tenantCarrier}  />
            </div>
