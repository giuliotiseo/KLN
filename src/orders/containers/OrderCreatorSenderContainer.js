...
import { useCustomerByCompanyIdQuery, useCustomerByIdQuery } from '../../customers/api/customers-api-slice';
...
import OrderDepotForSenderCompiler from '../components/form/OrderDepotForSenderCompiler';
...
// Actions
import { changeOrderCreatorCarrier, changeOrderCreatorCheckpoint, changeOrderCreatorCustomer, changeOrderCreatorForm, changeOrderCreatorPalletInfo, changeOrderCreatorPreOrder, changeOrderCreatorQuantity, changeOrderCreatorReceiver, changeOrderCreatorSender, changeOrderCreatorSize, changeOrderCreatorTrades, resetOrderCreator, selectOrderCreator } from '../slices/orderCreatorSlice';
import { selectCurrentCompany } from '../../company/slices/companySlice';
// Helpers
import { round } from '../../globals/libs/helpers';

function OrderCreatorSenderContainer() {
  ...
  const preOrdersQuery = useListPreOrders({ companyType: "SENDER", status: "ORDER" });
  ...
  const carrierDetailsQuery = useCustomerByCompanyIdQuery(order?.carrier?.id ? { companyId: order?.carrier?.company?.id || order?.carrier?.id }  : null );
  ...
  */
  ...
  const updateCarrier = useCallback((value) => {
    dispatch(changeOrderCreatorCarrier(value));
  }, [dispatch]);
    ...
    <div className="flex flex-col h-full w-full gap-4">      
      ...
      <div className="flex flex-col md:flex-row h-full">
        <aside className="relative rounded-lg flex-1 my-2 overflow-hidden">
          <OrderPickupCompiler
            ...
            customersQuery={carriersQuery}
            customerDetailsQuery={currentCompany}
            ordersInPreOrderQuery={ordersInPreOrderQuery}
            setCustomerSearchable={setCarrierSearchable}
            setPreOrderDrawer={setPreOrderDrawer}
            updateForm={updateForm}
            updateCustomer={updateCarrier}
        ...
        <section className="relative flex-1 lg:flex-2 bg-base-200">
          <SafeCol>
            <div className='px-4'>
              ...
              <OrderDepotForSenderCompiler
                order={order}
                customersQuery={carriersQuery}
                customerDetailsQuery={carrierDetailsQuery}
      ...
      <footer className="footer">
        <h5 className="title-5 uppercase opacity-50 flex items-center">
          ...
          <span className="text-sm">Compilazione ordine / MITTENTE</span>
        </h5>
...
export default OrderCreatorSenderContainer
